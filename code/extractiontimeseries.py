        ...
        if current_level == 4:  # 第四层  
            ...
            for item in os.listdir(dir_path):  
                ...
                if current_level == 1:  # 第一层，检查前缀  
                    ...
                    if item.startswith('ses-3') and os.path.isdir(os.path.join(dir_path, item)):  
                        yield from _find_files_recursive(os.path.join(dir_path, item), current_level + 1, target_suffixes)  
...
# 保存为pickle文件
with open("time_series_test_new.pkl", "wb") as f:
    pickle.dump(time_series_new, f)

# 读取pickle文件
with open("time_series_test_new.pkl", "rb") as f:
