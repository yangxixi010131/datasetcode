...
# Function to block a specific brain region
def block_region(matrix, region, brain_regions):
    indices = brain_regions[region]
    # Ensure the input matrix is a 2D NumPy array
    matrix = np.array(matrix)
    if matrix.ndim == 1:
        matrix = matrix[:, np.newaxis]  # Convert it to a 2D array
    blocked_matrix = np.copy(matrix)  # Use NumPy's copy method
...
# Function to create data objects and split training/testing sets
...
def main():
    ...
    # Perform sensitivity analysis for each brain region multiple times
    for region in brain_regions.keys():
        for _ in range(10):  # Repeat multiple times to enhance reliability
            # Convert data.x to a NumPy array and apply the block_region function
            ...
            data.x = x_train_blocked.view(-1, 116*116)  # Ensure the data shape is correct
    ...
    # Plot boxplot
    plt.figure(figsize=(10, 6))  # Adjust figure size to make boxplot narrower
