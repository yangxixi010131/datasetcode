...
for i, (region_name, region_indices) in enumerate(brain_regions.items()):
    ...
    plotting.plot_connectome(
        ...
        colorbar=False,  # Do not add color bar to show connection strength
        display_mode='ortho',  # Show sagittal, axial and coronal views
        black_bg=False,  # Use white background
        axes=axs.flat[i]  # Plot on corresponding subplot
    )
    
    # Use fig.text method to manually add centered title
    # Calculate position for title above each subplot
    pos = axs.flat[i].get_position()
    x = 0.5 * (pos.x0 + pos.x1)
    y = pos.y1 + 0.05  # Leave some space above subplot
