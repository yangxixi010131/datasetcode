...
def block_region(matrix, region, brain_regions):
    ...
    if matrix.ndim == 3:  # Check if it is a 3D array
        for i in range(matrix.shape[0]):
            for j in range(matrix.shape[1]):
                if i in indices or j in indices:
                    blocked_matrix[i, j] = 0
    elif matrix.ndim == 2:  # 2D array
        for i in range(matrix.shape[0]):
            for j in range(matrix.shape[1]):
                if i in indices or j in indices:
                    blocked_matrix[i, j] = 0
...
def main():
    ...
    for region in brain_regions.keys():
        for _ in range(10):  # Repeat multiple times to enhance the reliability of the results
            ...
            X_blocked = X_blocked.reshape(-1, 116*116)  # Convert the 3D matrix back to a 2D tensor

            scaler = StandardScaler()
            X_blocked = scaler.fit_transform(X_blocked)  # Standardize the data
    ...
    delta_accuracies = {region: abs(full_accuracy - mean_acc) for region, mean_acc in mean_accuracies.items()}  # Take the absolute value
    ...
    plt.yticks(fontsize=20)  # Adjust the font size of the y-axis ticks
